...
public class TransitionReorientCommand extends EditElementCommand {
	...
	*/
	protected boolean canReorientSource() {
		...
		Node target = getLink().getTrg();
		if (!(getLink().eContainer() instanceof Flowchart)) {
			return false;
		}
		Flowchart container = (Flowchart) getLink().eContainer();
		return FlowchartBaseItemSemanticEditPolicy.getLinkConstraints().canExistTransition_4001(container, getLink(),
				getNewSource(), target);
	}
